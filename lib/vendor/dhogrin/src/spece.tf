...
/def _especuj = \
  ...
  /let _spec_co_robi=$[{_spec_ile}!="0" ? "specuje" : "probuje specowac"]%;\
    ...
    /endif%; \
;  /let _spec_koment=$[strlen({7})>0 ? strcat(" (",{7},")") : ""]%;\
  /return echo(strcat(decode_attr(strcat("@{BCwhite}-------> ",{_spec_kolor},"[",{_spec_skrot}," ",{_spec_ile},"/",{_spec_na_ile},"] @{Cyellow}",{_spec_kto}," @{BCwhite}",{_spec_co_robi}," @{Cblue}",{_spec_kogo},"@{Cwhite}",{_spec_koment},"."))))
...
; Barbarzyńcy spec
; Aktualizacja: 25.12.12
/set _spec_kolor_barbarzyncy=@{Cred}
; obijajac, kaleczac, tlukac, gruchoczac, druzgoczac, miazdzac
; Ktoś kogoś
/def -aLg -Fp200 -mregexp -t'(Nie przebierajac w srodkach|Nie silac sie na wirtuozerie|Nie tracac czasu na zbedne manewry|Nie zastanawiajac sie dlugo) (.*) z calej sily wali (.*) na odlew (.*), jednak bron traci caly impet na (.*).' _barbarzyncy_spec_0_v1 = \
  /return (_especuj("BARB",{_spec_kolor_barbarzyncy}, {P2}, {P3}, "0", "6",{P5}))
/def -aLg -Fp200 -mregexp -t'(Nie przebierajac w srodkach|Nie silac sie na wirtuozerie|Nie tracac czasu na zbedne manewry|Nie zastanawiajac sie dlugo) (.*) z calej sily wali (.*) na odlew (.*), obijajac [^ ]+ przy tym (.*)\.' _barbarzyncy_spec_1 = \
  /return (_especuj("BARB",{_spec_kolor_barbarzyncy}, {P2}, {P3}, "1", "6",{P5}))
/def -aLg -Fp200 -mregexp -t'(Nie przebierajac w srodkach|Nie silac sie na wirtuozerie|Nie tracac czasu na zbedne manewry|Nie zastanawiajac sie dlugo) (.*) z calej sily wali (.*) na odlew (.*), kaleczac [^ ]+ przy tym (.*)\.' _barbarzyncy_spec_2 = \
  /return (_especuj("BARB",{_spec_kolor_barbarzyncy}, {P2}, {P3}, "2", "6",{P5}))
/def -aLg -Fp200 -mregexp -t'(Nie przebierajac w srodkach|Nie silac sie na wirtuozerie|Nie tracac czasu na zbedne manewry|Nie zastanawiajac sie dlugo) (.*) z calej sily wali (.*) na odlew (.*), tlukac [^ ]+ przy tym (.*)\.' _barbarzyncy_spec_3 = \
  /return (_especuj("BARB",{_spec_kolor_barbarzyncy}, {P2}, {P3}, "3", "6",{P5}))
/def -aLg -Fp200 -mregexp -t'(Nie przebierajac w srodkach|Nie silac sie na wirtuozerie|Nie tracac czasu na zbedne manewry|Nie zastanawiajac sie dlugo) (.*) z calej sily wali (.*) na odlew (.*), gruchoczac [^ ]+ przy tym (.*)\.' _barbarzyncy_spec_4 = \
  /return (_especuj("BARB",{_spec_kolor_barbarzyncy}, {P2}, {P3}, "4", "6",{P5}))
/def -aLg -Fp200 -mregexp -t'(Nie przebierajac w srodkach|Nie silac sie na wirtuozerie|Nie tracac czasu na zbedne manewry|Nie zastanawiajac sie dlugo) (.*) z calej sily wali (.*) na odlew (.*), druzgoczac [^ ]+ przy tym (.*)\.' _barbarzyncy_spec_5 = \
  /return (_especuj("BARB",{_spec_kolor_barbarzyncy}, {P2}, {P3}, "5", "6",{P5}))  
/def -aLg -Fp200 -mregexp -t'(Nie przebierajac w srodkach|Nie silac sie na wirtuozerie|Nie tracac czasu na zbedne manewry|Nie zastanawiajac sie dlugo) (.*) z calej sily wali (.*) na odlew (.*), miazdzac [^ ]+ przy tym (.*)\.' _barbarzyncy_spec_6 = \
  /return (_especuj("BARB",{_spec_kolor_barbarzyncy}, {P2}, {P3}, "6", "6",{P5}))

/def -p199 -mregexp -t'Nieprzyjemne chrzakniecie i metny wzrok przeciwnika swiadcza, ze dobrze wymierzony cios musial trafic w jakies wrazliwe miejsce.' _spec_barbarzyncy_ogluszenie = \
  /echo -p @{n}@{BCwhite}   ----> @{Cred}[OGLUSZENIE]@{Cwhite} spec gratis.
...
; Ktoś mnie
/def -Fp100 -mregexp -t' wykorzystujac twoj brak broni,(| blyskawicznie| w mgnieniu oka| z wyuczona wprawa) wyprowadza cios (.*) mierzac w (.*) i prawie traci rownowage po [^ ]+ zrecznym uniku.' str_spec_we_mnie_0_unik = \
  /return (_specuj("STR",{_spec_kolor_straznika}, {PL}, {P3}, "0", "5", "unik"))
/def -Fp100 -mregexp -t' wykorzystujac twoj brak broni,(| blyskawicznie| w mgnieniu oka| z wyuczona wprawa) wyprowadza cios (.*) mierzac w (.*). Wypracowane uderzenie zostaje jednak sparowane przez [^ ]+ [^ ]+ [^ ]+ tarcza.' str_spec_we_mnie_0_tarcza = \
  /return (_specuj("STR",{_spec_kolor_straznika}, {PL}, {P3}, "0", "5", "tarcza"))
/def -Fp100 -mregexp -t' wykorzystujac twoj brak broni,(| blyskawicznie| w mgnieniu oka| z wyuczona wprawa) wyprowadza cios (.*) mierzac w (.*). Wypracowane uderzenie trafia cie w (.*), (pozostawiajac ledwie sinca|ledwie zahaczajac).' str_spec_we_mnie_1 = \
  /return (_specuj("STR",{_spec_kolor_straznika}, {PL}, {P3}, "1", "5", {P4}))
/def -Fp100 -mregexp -t' wykorzystujac twoj brak broni,(| blyskawicznie| w mgnieniu oka| z wyuczona wprawa) wyprowadza cios (.*) mierzac w (.*). Wypracowane uderzenie trafia cie w (.*), (pozostawiajac krwawiace zadrapanie|lekko raniac).' str_spec_we_mnie_2 = \
  /return (_specuj("STR",{_spec_kolor_straznika}, {PL}, {P3}, "2", "5", {P4}))
/def -Fp100 -mregexp -t' wykorzystujac twoj brak broni,(| blyskawicznie| w mgnieniu oka| z wyuczona wprawa) wyprowadza cios (.*) mierzac w (.*). Wypracowane uderzenie trafia cie w (.*), (pozostawiajac dluga, poszarpana rane|mocno raniac).' str_spec_we_mnie_3 = \
  /return (_specuj("STR",{_spec_kolor_straznika}, {PL}, {P3}, "3", "5", {P4}))
/def -Fp100 -mregexp -t' wykorzystujac twoj brak broni,(| blyskawicznie| w mgnieniu oka| z wyuczona wprawa) wyprowadza cios (.*) mierzac w (.*). Wypracowane uderzenie trafia cie w (.*), (pozostawiajac gleboka rane z wystajacymi na wierzchu tkankami|bardzo mocno raniac).' str_spec_we_mnie_4 = \
  /return (_specuj("STR",{_spec_kolor_straznika}, {PL}, {P3}, "4", "5", {P4}))
/def -Fp100 -mregexp -t' wykorzystujac twoj brak broni,(| blyskawicznie| w mgnieniu oka| z wyuczona wprawa) wyprowadza cios (.*) mierzac w (.*). Wypracowane uderzenie trafia cie w (.*), zadajac smiertelne obrazenia.' str_spec_we_mnie_5 = \
...
/def -Fp100 -mregexp -t'( zrecznym ruchem paruje cios (.*), rownoczesnie zachodzac [^ ]+ od tylu. Starannie wymierzonym pchnieciem wbija (.*) w [^ ]+ plecy, tuz miedzy lopatkami. Oczy (.*) rozszeraja sie z bolu, gdy osuwa sie [^ ]+ z cichym jekiem na ziemie. Dla [^ ]+ to juz koniec walki\.|Unikajac uderzenia (.*), (.*) blyskawicznie doskakuje do [^ ]+ i wbija az po rekojesc swoj (.*) w [^ ]+ cialo. Na chwile zamiera w bezruchu, z satysfakcja obserwujac, jak z [^ ]+ ofiary uchodzi zycie. (.*) z cichym jekiem osuwa sie na ziemie, a (zabojca|zabojczyni) wyszarpuje swa bron z ciala i noga odsuwa zwloki na bok, rozgladajac sie za kolejnymi przeciwnikami\.|Wykorzystujac dekoncentracje i oslabienie (.*), (.*) silnym kopniakiem podcina [^ ]+, rownoczesnie wyrzucajac do przodu swa reke z (.*)\. Przeciwnik traci rownowage i osuwa sie na [^ ]+ bron, ta zas zaglebia sie w [^ ]+ cialo, odbierajac [^ ]+ ostatnie sily. (.*) z zimnym usmiechem wyciaga zakrwawiony (.*) z osuwajacego sie na ziemie ciala\.)' ra_spec_8 = \
  /return (_specuj("RA",{_spec_kolor_noz}, strcat({PL},{P6},{P11}), strcat({P2},{P5},{P10}), "8", "8"))
...
; Miecznik 
; aktualizacja 12.04.13
/set _spec_kolor_miecznika=@{Cmagenta}

/def -p99 -mregexp -t'(Odpowiednio przenoszac ciezar ciala ciagnie cios dalej,|Przykleknawszy na jedno kolano prowadzi cios dalej,|Wykorzystujac dynamike ciosu przeciaga go jeszcze,|Korzystajac z dogodnego ustawienia przeciaga jeszcze cios,) muskajac (.*) samym sztychem broni.' miecznik+1 = \
  /echo -p @{BCwhite}  -------> @{Cmagenta}[MIECZ+ 1/7]@{Cwhite} spec gratis w @{Cblue}%{P2}@{CWhite}.
/def -p99 -mregexp -t'(Odpowiednio przenoszac ciezar ciala ciagnie cios dalej,|Przykleknawszy na jedno kolano prowadzi cios dalej,|Wykorzystujac dynamike ciosu przeciaga go jeszcze,|Korzystajac z dogodnego ustawienia przeciaga jeszcze cios,) lekko raniac (.*) samym sztychem broni.' miecznik+2 = \
  /echo -p @{BCwhite}  -------> @{Cmagenta}[MIECZ+ 2/7]@{Cwhite} spec gratis w @{Cblue}%{P2}@{CWhite}.
/def -p99 -mregexp -t'(Odpowiednio przenoszac ciezar ciala ciagnie cios dalej,|Przykleknawszy na jedno kolano prowadzi cios dalej,|Wykorzystujac dynamike ciosu przeciaga go jeszcze,|Korzystajac z dogodnego ustawienia przeciaga jeszcze cios,) raniac (.*) samym sztychem broni.' miecznik+3 = \
  /echo -p @{BCwhite}  -------> @{Cmagenta}[MIECZ+ 3/7]@{Cwhite} spec gratis w @{Cblue}%{P2}@{CWhite}.

/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore tylko jakims cudem nie dosiega ([^.]*).' miecznik_0 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "0", "7"))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore tylko cudem nie dosiega twej twarzy.' miecznik_0_wemnie = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), "ciebie", "0", "7"))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore szczesliwie zeslizguje sie po' miecznik_0_zbroja_wemnie = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), "ciebie", "0", "7"))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore jednak zeslizguje sie po [^ ]+ [^ ]+ [^ ]+ (.*)' miecznik_0_zbroja = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "0", "7"))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore ledwo zacina (.*) w ([^.]*).' miecznik_1 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "1", "7",{P4})) 
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore bolesnie zacina (.*) w ([^.]*).' miecznik_2 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "2", "7",{P4}))
/def -aLg -Fp101 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore plytko tnie (.*) w ([^.]*).' miecznik_3 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "3", "7",{P4}))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore tnie (.*) w ([^.]*).' miecznik_4 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "4", "7",{P4}))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore gleboko tnie (.*) w ([^.]*).' miecznik_5 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "5", "7",{P4}))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore niemalze rozcina (.*) (prawe ramie|lewe ramie|nogi|korpus|glowe).' miecznik_6 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "6", "7",{P4}))
/def -aLg -Fp100 -mregexp -t'( wyrzuca do przodu swoj| rusza do przodu,| poprawia chwyt na swym| niespodziewanie skreca tulow,|Nie zwazajac na nic (.*) rozpoczyna) .*, ktore z potworna moca tnie (.*) doslownie w pol.' miecznik_7 = \
  /return (_especuj("MIECZNIK",{_spec_kolor_miecznika}, strcat({PL},{P2}), {P3}, "7", "7"))

#######################################################################################################################################################

; Spec korsarzy
; Aktualizacja 14.02.13
...
/def -p200 -mregexp -t'Niezgrabnym ruchem (.*) unika (twego ciosu|ciosu (.*)), a [^ ]+ ((.*) wyrzucony w bok|wyrzucony w bok (.*),) bardziej dla utrzymania rownowagi niz zadania ciosu, przecina tylko powietrze.' _korek_spec_0 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {P1}, {P3-ciebie}, "0", "5", "pudlo"))
/def -p200 -mregexp -t' niezgrabnym ruchem probuje zaskoczyc (.*), ale [^ ]+ wyrzucony przed siebie (.*) dzga tylko powietrze\.' _korek_spec_0_v2 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, {P1}, "0", "5", "pudlo"))
/def -p200 -mregexp -t' powolnym ruchem unika (twego ciosu|ciosu (.*)), jednoczesnie uderzajac ((cie)|[^ ]+) (swym|swoja) (.*), lecz niedogodna pozycja nie pozwala [^ ]+ na zbyt silny cios i zadaje (ci |)tylko nieznaczne obrazenia.' _korek_spec_1 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, strcat({P2},{P4}), "1", "5"))
/def -p200 -mregexp -t' powolnym wypadem do przodu atakuje (.*) dzgajac [^ ]+ (swym|swoja) (.*), lecz niedogodna pozycja nie pozwala [^ ]+ na zbyt silny cios i zadaje tylko nieznaczne obrazenia.' _korek_spec_1_v2 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, {P1}, "1", "5"))
/def -p200 -mregexp -t' oszczednym ruchem unika (twego ciosu ,|ciosu (.*),) rabiac ((cie)|[^ ]+) rownoczesnie (swym|swoja) (.*), a uderzenie to otwiera szeroka rane na (ciele jego przeciwnika|twoim ciele).' _korek_spec_2 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, strcat({P2},{P4}), "2", "5"))
/def -p200 -mregexp -t' oszczednym wypadem do przodu atakuje (.*), dzgajac [^ ]+ (swym|swoja) (.*), a uderzenie to pozostawia gleboka rane w ciele [^ ]+ przeciwnika\.' _korek_spec_2_v2 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, {P1}, "2", "5"))
/def -p200 -mregexp -t' zgrabnym ruchem unika (twego ciosu|ciosu (.*)) i korzystajac z dogodnej pozycji rabie ((cie)|[^ ]+) na odlew (swym|swoja) (.*), gleboko rozrywajac [^ ]+ cialo.' _korek_spec_3 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, strcat({P2},{P4}), "3", "5"))
/def -p200 -mregexp -t' zgrabnym wypadem do przodu atakuje (.*) i korzystajac z zaskoczenia dzga [^ ]+ mocno (swym|swoja) (.*), gleboko wbijajac sie w [^ ]+ cialo.' _korek_spec_3_v2 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, {P1}, "3", "5"))
/def -p200 -mregexp -t' plynnym ruchem unika (twego ciosu|ciosu (.*)) i uderzejac ze wszystkich sil tnie ((cie)|[^ ]+) (swym|swoja) (.*), (ktory|ktora) gleboko rozrywa [^ ]+ cialo, powodujac ogromne obrazenia.' _korek_spec_4 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, strcat({P2},{P4}), "4", "5"))
/def -p200 -mregexp -t' plynnym wypadem do przodu atakuje (.*) i napierajac calym cialem dzga [^ ]+ (swym|swoja) (.*), (ktory|ktora) przechodzi prawie na wylot przez [^ ]+ cialo, powodujac ogromne obrazenia.' _korek_spec_4_v2 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, {P1}, "4", "5"))
/def -p200 -mregexp -t' blyskawicznym polobrotem unika (twego ciosu|ciosu (.*)), a [^ ]+ (.*), dzierzon[y|a] w (wyciagnietej rece|wyciagnietych rekach), zatoczywszy szeroki luk uderza w ((cie)|[^ ]+) cialo i niemal kompletnie je rozdzierajac, wynurza sie z drugiej strony ciagnac za soba krwawy warkocz smierci.' _korek_spec_5 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, strcat({P2},{P6}), "5", "5"))
/def -p200 -mregexp -t' blyskawicznym polobrotem unika (twego ciosu|ciosu (.*)), a [^ ]+ (.*), dzierzon[y|a] w (wyciagnietej rece|wyciagnietych rekach), zatoczywszy szeroki luk uderza w ((cie)|[^ ]+) cialo i niemal kompletnie je masakrujac powala [^ ]+ na ziemie.' _korek_spec_5_v3 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, strcat({P2},{P6}), "5", "5"))
/def -p200 -mregexp -t' blyskawicznym wypadem do przodu atakuje (.*), a [^ ]+ (.*), dzierzon[y|a] w wyciagnietej rece, trafia w [^ ]+ cialo przebijajac je na wylot. Gwaltownie wyszarpniety orez z glosnym mlasnieciem wychodzi z ciala ciagnac za soba krwawy warkocz smierci.' _korek_spec_5_v2 = \
  /return (_specuj("KS",{_spec_kolor_bs}, {PL}, {P1}, "5", "5"))
...
/def -Fp100 -mregexp -t'Niezgrabny ruch (.*) niemal cudem pozwala [^ ]+ uniknac (twego ciosu|ciosu (.*)), a [^ ]+ wyrzucony w bok (.*), bardziej dla utrzymania rownowagi niz zadania ciosu, przecina tylko powietrze.' _berek_spec_0 = \
  /return (_specuj("BS",{_spec_kolor_bs}, {P1}, {P3-ciebie}, "0", "5", "pudlo"))
/def -Fp100 -mregexp -t' plynnym ruchem unika ciosu (.*) i uderzejac ze wszystkich sil tnie [^ ]+, swym (.*), ktory wsrod chrzestu miazdzonych kosci rozrywa [^ ]+ cialo, powodujac ogromne obrazenia.' _berek_spec_4 = \
  /return (_specuj("BS",{_spec_kolor_bs}, {PL}, {P1}, "4", "5"))
/def -Fp100 -mregexp -t' blyskawicznym polobrotem unika ciosu (.*), a [^ ]+ (.*), dzierzon[^ ]+ w (wyciagnietej rece|wyciagnietych rekach), zatoczywszy szeroki luk uderza u nasady [^ ]+ czaszki i niemal oddzieliwszy glowe od tulowia, wynurza sie z drugiej strony ciagnac za soba krwawy warkocz smierci.' _berek_spec_5 = \
