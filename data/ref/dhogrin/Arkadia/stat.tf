...
;    /_zapisz
/def -p10 -F -mregexp -t'^Zabil[ea]s (.+)\\.$' zabiles_cosik_licz = \
...
/def sets_stats = \
	/set statsid=1%;\
	/quote -S /test set_stat('"tf/moje/stats.db")%;\
	/set statsid=1
/def set_stat = \
	/set _stat_%{statsid}=%{*}%;\
        /test ++statsid
/def -i doeach = \
  ...
  /done 
/def zapiszstaty = \
  /if ((uchwyt:=tfopen("tf/moje/stats.db", "w"))<0) \
    /return 0%; \
    /echo Blad zapisu licznika ofiar do pliku "tf/moje/stats.db"!%; \
  /endif%;\
...
/def -mregexp -t'^Probujesz trafic '               			      stats_1 = /test ++_stat_1
/def -mregexp -t'^Trafiasz (.*), lecz caly impet '    			      stats_2 = /test ++_stat_2
/def -mregexp -t'^(Wyprowadzasz szybkie pchniecie|Robisz zamach|Wykonujesz zamaszyste ciecie) .*\, lecz .* (zbija|paruje)'          stats_3 = /test ++_stat_3
/def -mregexp -t'^(Wyprowadzasz szybkie pchniecie|Robisz zamach|Wykonujesz zamaszyste ciecie) .*\, lecz .* oslania'        stats_4 = /test ++_stat_4
/def -mregexp -t'^Ledwo muskasz '          				      stats_5 = /test ++_stat_5
/def -mregexp -t'^Lekko ranisz '                                              stats_6 = /test ++_stat_6
/def -mregexp -t'^Ranisz '	                                              stats_7 = /test ++_stat_7
/def -mregexp -t'^Powaznie ranisz '                                           stats_8 = /test ++_stat_8
/def -mregexp -t'^Bardzo ciezko ranisz '                                      stats_9 = /test ++_stat_9
/def -mregexp -t'^Masakrujesz '                                               stats_10 = /test ++_stat_10
/def -mregexp -t'probuje cie trafic .* lecz tobie udaje sie uniknac '         stats_19 = /test ++_stat_19
/def -mregexp -t'(lecz tobie udaje sie oslonic|lecz udaje ci sie oslonic)'     stats_20 = /test ++_stat_20
/def -mregexp -t'(lecz tobie udaje sie je zbic|lecz tobie udaje sie go sparowac|lecz tobie udaje sie zbic je)' stats_21 = /test ++_stat_21
/def -mregexp -t'trafia cie (.*), lecz caly impet uderzenia zostaje'           stats_22 = /test ++_stat_22
/def -mregexp -t' ledwo muska cie ' 					      stats_23 = /test ++_stat_23
/def -mregexp -t' lekko rani cie '					      stats_24 = /test ++_stat_24
/def -mregexp -t' ((?<!lekko |powaznie |bardzo ciezko )rani) cie '            stats_25 = /test ++_stat_25
/def -mregexp -t' powaznie rani cie '					      stats_26 = /test ++_stat_26
/def -mregexp -t' bardzo ciezko rani cie '				      stats_27 = /test ++_stat_27
/def -mregexp -t' masakruje cie ' 					      stats_28 = /test ++_stat_28
