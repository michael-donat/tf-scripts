...
; ## 
; ## Wersja pliku                 : 1.0.6
; ## Ostatnia zmiana pliku        : 2005-01-04 22:26:39
; ## Plik pobrany                 : 2005-01-04 22:37:22
; ## 
; ##
...
/def -E({color_log_on}) -F -mregexp -p0 -t'^' _loguj_color = \
...
;        /echo Set font %{_font} for %{_color} %;\
        /let _c=$[strcat("</span><span style='color: ", _font, "; background-color: ", _bgfont, "; " , _underline, "'>")] %;\
;        /echo Color: %_color BgColor: %_bgcolor Bold: %_bold font: %_font bgfont %_bgfont%;\
        /let _out=$[strcat(_out, _c)] %;\
    /done %;\
    /let _out=$[strcat(_out, _line)] %;\
;    /test tfwrite({_ch}, strcat({_out}))
    /test tfwrite({_ch}, strcat(replace("  ", " &nbsp;", {_out}), "<br />"))
;    /substitute 
    

/def -mregexp -p20 -t'^(\\[.{3}\\]\\[.{14}\\] |>> .* SPEC << )' _rem_kond = /return substitute({PR})
/def -mregexp -p1000 -t'^\\d{2}:\\d{2}:\\d{2}\\.\\d{6} #( |)' _rem_time = /return substitute({PR})
...
;    /test tfwrite({_ch}, '<!ATTLIST body xml:space (default|preserve) 'preserve'>') %;\
    /test tfwrite({_ch}, '<html xmlns="http:\/\/www.w3.org/1999/xhtml" xml:lang="en" >') %;\
...
;    /test tfwrite({_ch}, '<pre xml:space='preserve'>') %;\
    ...
    /quote -S /set _a='"%{1}" %%; /test fake_recv(_a) %;\
