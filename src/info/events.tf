...
/eval /def -mregexp -t'.* %{_info_events_speech_type_list} do ciebie:? .*' -aCrgb315 -p15 _info_events_speech_1
/eval /def -mregexp -t'.* %{_info_events_speech_type_list}:?(do |) .*' -aCrgb025 -p10 _info_events_speech_2
